#!/bin/bash
# salmon_quant_pe0140.sh <path to data dir> <path to index dir> <path to salmon_quant_pe.cwl> <path to salmon_quant_pe.yaml.sample>
#
set -e
...
run_salmon_quant_pe(){
  local fpath="${1}"
  local id=$(basename "${fpath}" | sed 's:.fq.*$::g' | sed 's:_._val_.$::g')
...
config_yaml_paired_end(){
  local yaml_path="${1}"
  local path_fwd="${2}"
  local path_rev="$(echo "${path_fwd}" | sed 's:_1_val_1.fq:_2_val_2.fq:')"
